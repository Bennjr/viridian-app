...
type FileItem = { name: string; path: string; type: string; content?: string; modified: number; size: number; desc: string };
...
export default function LibraryFullGrid({ goDashboard }: { goDashboard: () => void }) {
    ...
    useEffect(() => {
        ...
        const fetch = async () => {
            ...
            try {
                ...
                if (isCurrent) setFiles(Array.isArray(data) ? data : []);
            } finally {
    ...
    return (
        <div className="flex h-full w-full bg-c-primary overflow-hidden">
            <main className="flex-1 flex flex-col min-w-0 h-full border-r border-white/5 overflow-hidden">
                <header className="h-16 flex items-center justify-between px-6 bg-c-primary/50 backdrop-blur-md border-b border-white/5 shrink-0">
                    <div className="flex items-center gap-4 flex-1">
                        <button onClick={goDashboard} className="p-2 hover:bg-c-secondary rounded-full transition-colors">
                        ...
                        <div className="relative w-full max-w-md group">
                            ...
                            {isLoading && (
                                <div className="absolute right-3 top-1/2 -translate-y-1/2">
                                    <div className="size-3 border-2 border-c-brand border-t-transparent rounded-full animate-spin"></div>
                                </div>
                            )}
                        </div>
                    </div>
                        ...
                        {isLoading ? (
                            ...
                            files.map((file) => (
                                <FileBentoCard
                                    ...
                                    onToggleFav={() => toggleFav(file.path)}
                                />
                            ))
                        )}
                    </motion.div>
            ...
            <aside className="w-72 bg-c-secondary flex flex-col border-l border-white/5 shadow-2xl h-full shrink-0 overflow-hidden">
                {selectedFile ? (
                    <InspectorView
                        file={selectedFile}
                        isFav={favorites.has(selectedFile.path)}
                        onToggleFav={() => toggleFav(selectedFile.path)}
...
function FileSkeleton() {
    return (
        <div className="p-4 rounded-2xl border border-white/5 bg-c-tertiery animate-pulse">
            <div className="flex justify-between items-start mb-6">
                <div className="p-2 rounded-xl bg-white/5 size-9" />
                <div className="p-1.5 rounded-lg bg-white/5 size-6" />
            </div>
...
function FileBentoCard({ file, isActive, isFav, onClick, onToggleFav }: any) {
    return (
        ...
        >
            ...
            {isActive && <div className="absolute top-2 right-2 size-1 rounded-full bg-c-brand animate-pulse" />}
        </motion.div>
    );
}
...
function InspectorView({ file, isFav, onToggleFav, onClose }: any) {
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden animate-in slide-in-from-right duration-300">
            <div className="p-4 flex justify-between items-center border-b border-white/5 bg-white/[0.01] shrink-0">
            ...
            <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
                <div className="aspect-[4/3] bg-c-primary rounded-2xl mb-6 border border-white/5 flex flex-col items-center justify-center relative overflow-hidden shrink-0">
                    <Icon src="/file-text.svg" size="w-10 h-10" className="opacity-10" />
                </div>

                <div className="flex justify-between items-start gap-4 mb-8">
                    <div className="min-w-0">
                        <h2 className="text-sm font-black text-white truncate leading-tight mb-1">{file.name}</h2>
                        <p className="text-[9px] font-mono opacity-20 break-all">{file.path}</p>
                    </div>
                ...
                <div className="mt-10 grid grid-cols-2 gap-2">
                    <button onClick={() => invoke("show_in_folder", { path: file.path })} className="flex-1 bg-c-tertiery hover:bg-white/5 border border-white/5 py-2.5 rounded-lg text-[9px] font-black tracking-widest transition-all uppercase">
                        Path
                    </button>
...
function GlobalStatsView({ files }: { files: any[] }) {
    return (
        <div className="p-6 flex flex-col gap-8 h-full overflow-hidden">
            <div>
                <label className="text-[10px] font-black tracking-[0.2em] text-white/20 uppercase block mb-4 text-left">Quick Filters</label>
            ...
            <div className="mt-auto opacity-20 text-[9px] font-black uppercase tracking-widest text-left">
                Files: {files.length}
            </div>
        </div>
    );
...
function DetailRow({ label, value, color = "text-white/40" }: any) {
    return (
        <div className="flex justify-between items-center py-3 border-b border-white/[0.02]">
            <span className="text-[9px] font-bold text-white/20 uppercase tracking-tighter text-left">{label}</span>
            <span className={`text-[10px] font-bold ${color} text-right`}>{value}</span>
        </div>
    );
}